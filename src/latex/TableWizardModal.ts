...
export class TableWizardModal extends Modal {
	...
	onOpen(): void {
		...
		new Setting(contentEl).setName('Format').addDropdown((dd) =>
			dd
				...
				.onChange((v: TableFormat) => {
					this.format = v;
					this.updatePreview();
				}),
		);

		new Setting(contentEl).setName('Columns').addSlider((s) =>
			s
				.setLimits(1, 10, 1)
		...
		new Setting(contentEl).setName('Include Header').addToggle((t) =>
			t.setValue(this.includeHeader).onChange((v) => {
				this.includeHeader = v;
				this.updatePreview();
			}),
		);
		...
		const csvDesc = contentEl.createEl('p', {
			text: 'Paste comma- or tab-separated values to auto-fill headers and rows.',
		});
		csvDesc.addClass('setting-item-description');
		new Setting(contentEl).setName('Data').addTextArea((ta) => {
		...
		new Setting(contentEl)
			...
			.addToggle((tg) =>
				tg.setValue(this.csvHeaderFromFirstRow).onChange((v) => {
					this.csvHeaderFromFirstRow = v;
					this.updatePreview();
				}),
			);
		...
		new Setting(contentEl)
			...
			.addButton((b) =>
				b.setButtonText('Guess').onClick(() => {
					this.guessAlignmentFromCSV();
					this.renderColumnsSection();
					this.updatePreview();
				}),
			);
		...
		new Setting(contentEl).setName('Caption').addText((t) =>
			t.setValue(this.caption).onChange((v) => {
				this.caption = v;
				this.updatePreview();
			}),
		);
		new Setting(contentEl).setName('Label (LaTeX)').addText((t) =>
			t
				.setPlaceholder('my-table')
				.setValue(this.label)
				.onChange((v) => {
					this.label = v;
					this.updatePreview();
		...
		new Setting(contentEl)
			...
			.addToggle((tg) =>
				tg.setValue(this.latexUseBooktabs).onChange((v) => {
					this.latexUseBooktabs = v;
					if (v) this.latexUseHlines = false;
					this.updatePreview();
				}),
			);
		...
		new Setting(contentEl)
			...
			.addToggle((tg) =>
				tg.setValue(this.latexUseHlines).onChange((v) => {
					this.latexUseHlines = v;
					if (v) this.latexUseBooktabs = false;
					this.updatePreview();
				}),
			);
		...
		new Setting(contentEl)
			...
			.addToggle((tg) =>
				tg.setValue(this.latexVerticalBars).onChange((v) => {
					this.latexVerticalBars = v;
					this.updatePreview();
				}),
			);
		...
		new Setting(contentEl)
			...
			.addToggle((tg) =>
				tg.setValue(this.latexIncludeCentering).onChange((v) => {
					this.latexIncludeCentering = v;
					this.updatePreview();
				}),
			);
		...
		new Setting(previewBtns).addButton((b) =>
			b.setButtonText('Copy Preview').onClick(async () => {
				try {
					await navigator.clipboard.writeText(this.previewEl?.innerText || '');
				} catch (_) {
					/* intentionally empty */
				}
			}),
	...
	private buildMarkdown(): string {
		...
		let headerLine = `| ${Array.from({ length: cols })
			.map((_, i) => this.headers[i] || `Col ${i + 1}`)
			.join(' | ')} |`;
		...
		if (this.csvData && this.csvData.length > 0) {
			...
			if (this.includeHeader) {
				if (this.csvHeaderFromFirstRow) {
					...
					headerLine = `| ${Array.from({ length: cols })
						.map((_, i) => h[i] ?? '')
						.join(' | ')} |`;
				} else {
					headerLine = `| ${Array.from({ length: cols })
	...
	private buildLatex(): string {
		...
		let colSpec = this.alignments
			.slice(0, this.columns)
			.map((a) => alignMap[a])
			.join('');
		...
		if (this.csvData && this.csvData.length > 0) {
			...
			const blank = `${Array.from({ length: this.columns })
				.map(() => '')
				.join(' & ')} \\`;
