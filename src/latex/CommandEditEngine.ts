 ...
 */
export function detectCommandOnLine(lineText: string, cursorCh: number): { match: RegExpMatchArray; index: number } | null {
...
export function mapMatchToSpec(match: RegExpMatchArray): { spec?: CommandSpec; values: Record<string, string> } {
  ...
  const optKey = spec.args.find((a) => a.kind === 'optional')?.key || spec.args.find((a) => a.kind === 'optional')?.name;
  if (opt && optKey) {
