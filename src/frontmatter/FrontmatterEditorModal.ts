...
export class FrontmatterEditorModal extends Modal {
	...
	async onOpen() {
		...
		new Setting(contentEl)
			.setName('Title')
			.addText((t) => t.setValue(this.fields.title || '').onChange((v) => (this.fields.title = v)));
		...
		new Setting(contentEl)
			.setName('Date')
			.setDesc('YYYY-MM-DD or free text')
			.addText((t) => t.setValue(this.fields.date || '').onChange((v) => (this.fields.date = v)));

		new Setting(contentEl).setName('Abstract').addTextArea((ta) => {
		...
		new Setting(contentEl)
			...
			.addText((t) =>
				t.setValue((this.fields.keywords || []).join(', ')).onChange(
					(v) =>
						(this.fields.keywords = v
							.split(',')
							.map((s) => s.trim())
							.filter(Boolean)),
				),
		...
		const bibSetting = new Setting(contentEl)
			.setName('Bibliography')
			.setDesc('One .bib path per line or add via Browse');
		bibSetting.addTextArea((ta) => {
			ta.setValue((this.fields.bibliography || []).join('\n')).onChange(
				(v) =>
					(this.fields.bibliography = v
						.split(/\r?\n/)
						.map((s) => s.trim())
						.filter(Boolean)),
			);
		...
		cslSetting.addText((t) => {
			t.setValue(this.fields.csl || '').onChange((v) => {
				this.fields.csl = v;
				try {
					updateCslStatus();
				} catch (_) {
		...
		new Setting(varActions).addButton((b) =>
			b
				.setIcon('plus')
					...
					btn
						...
						.onClick(async () => {
							...
							if (reserved.has(cleaned)) {
								new Notice('Name is reserved');
								return;
							}
							if (this.fields.extraMeta[cleaned] !== undefined && cleaned !== k) {
