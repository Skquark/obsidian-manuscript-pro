...
export function updateFrontmatterYaml(originalYaml: string, updated: FrontmatterFields): string {
	...
	if (updated.bibliography !== undefined)
		replaceBlock('bibliography', writeArrayBlock('bibliography', updated.bibliography));
	if (updated.csl !== undefined) replaceBlock('csl', [`csl: ${quoteIfNeeded(updated.csl)}`]);
	if (updated.headerIncludes !== undefined)
		replaceBlock('header-includes', writeArrayBlock('header-includes', updated.headerIncludes));
...
export function buildFrontmatterYaml(fields: FrontmatterFields): string {
	...
	if (fields.keywords && fields.keywords.length) parts.push('keywords:', ...fields.keywords.map((k) => `  - ${k}`));
