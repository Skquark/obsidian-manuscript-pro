...
export class ReadabilityAnalyzer {
	...
	calculateMetrics(text: string): ReadabilityMetrics {
		...
		// SMOG Index
		const smogIndex = sentenceCount > 0 ? 1.043 * Math.sqrt(complexWords * (30 / sentenceCount)) + 3.1291 : 0;
		...
		// Average grade level
		const gradeLevel =
			(fleschKincaidGrade + gunningFogIndex + smogIndex + colemanLiauIndex + automatedReadabilityIndex) / 5;
	...
	private generateRecommendations(metrics: ReadabilityMetrics, target: AudienceProfile): string[] {
