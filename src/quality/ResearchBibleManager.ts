...
export class ResearchBibleManager {
  ...
  getFact(term: string): ResearchFact | undefined {
    return this.bible.facts.find(f =>
      f.term.toLowerCase() === term.toLowerCase()
    );
  }

  searchFacts(query: string): ResearchFact[] {
    const lowerQuery = query.toLowerCase();
    return this.bible.facts.filter(f =>
  ...
  getEntity(name: string): EntityInfo | undefined {
    // Check exact match
    let entity = this.bible.entities.get(name.toLowerCase());
    if (entity) return entity;

    // Check alternate names
    for (const [_, e] of this.bible.entities) {
  ...
  async checkTerminologyConsistency(): Promise<TerminologyConsistency[]> {
    ...
    for (const [term, definition] of this.bible.terminology) {
  ...
  private findAcronymInconsistencies(
    acronym: string,
    expansion: string,
    content: string,
    lines: string[]
  ...
  private isCommonWord(word: string): boolean {
    const common = ['THE', 'AND', 'FOR', 'ARE', 'BUT', 'NOT', 'YOU', 'ALL', 'CAN', 'HER', 'WAS', 'ONE', 'OUR', 'OUT', 'DAY'];
  ...
  exportGlossary(format: 'markdown' | 'latex' | 'html' = 'markdown'): string {
    const terms = Array.from(this.bible.terminology.entries()).sort((a, b) =>
  ...
  exportAcronymList(format: 'markdown' | 'latex' = 'markdown'): string {
    const acronyms = Array.from(this.bible.acronyms.entries()).sort((a, b) =>
      a[0].localeCompare(b[0])
    );
  ...
  exportEntityIndex(format: 'markdown' | 'json' = 'markdown'): string {
    const entities = Array.from(this.bible.entities.values()).sort((a, b) =>
  ...
  async importFromJSON(json: string): Promise<void> {
    try {
      ...
      if (data.entities) {
        this.bible.entities = new Map(
          data.entities.map((e: EntityInfo) => [e.name.toLowerCase(), e])
        );
      }
